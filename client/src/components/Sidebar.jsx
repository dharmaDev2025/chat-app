        ...
        <div className="flex justify-between items-center">
          ...
          <div className="relative py-2 group">
            ...
            <div className="absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#282142] border border-gray-600 text-gray-100 hidden group-hover:block">
              <p onClick={() => navigate('/profile')} className="cursor-pointer hover:text-purple-400">Edit Profile</p>
              <hr className="my-2 border-t border-gray-500"/>
              <p className="cursor-pointer text-sm hover:text-purple-400">Logout</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto flex flex-col space-y-2">
        {userDummyData.map((user, index) => (
          ...
          >
            ...
            {index > 2 && (
              <p className="absolute top-4 right-4 text-xs h-5 flex justify-center items-center rounded-full bg-violet-500/50">
                {index}
              </p>
            )}
          </div>
